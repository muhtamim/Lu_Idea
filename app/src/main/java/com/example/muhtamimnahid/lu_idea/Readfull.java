...
public class Readfull extends AppCompatActivity implements View.OnClickListener {
        ...
        mdatabaselikes.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if(dataSnapshot.child(key).hasChild(mAuth.getCurrentUser().getUid())){
                    Drawable d = getResources().getDrawable(R.drawable.upvotered);
                    blb.setBackground(d);
                    mdatabaselikes.child(key).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            ...
                            if(cnt==1){
                                tlb.setText(String.valueOf(cnt)+" Vote");
                            }
                            else{
                                tlb.setText(String.valueOf(cnt)+" Votes");
                            }
                        }
                        @Override
                        public void onCancelled(DatabaseError databaseError) {
                    ...
                    Drawable d = getResources().getDrawable(R.drawable.upvote);
                    blb.setBackground(d);
                    mdatabaselikes.child(key).addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(DataSnapshot dataSnapshot) {
    ...
    public void onClick(View v) {
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    if(mlikes) {
                        if (dataSnapshot.child(key).hasChild(mAuth.getCurrentUser().getUid())) {
                            ...
                            mdatabaselikes.child(key).child(mAuth.getCurrentUser().getUid()).setValue("user Voted");
                            mlikes = false;
                        }
                    }
                }
        ...
        if(v==bck){
            ...
            intent.setClassName(getApplicationContext(),"com.example.muhtamimnahid.lu_idea"+a);
